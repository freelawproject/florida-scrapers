...
const dedupeJSONResult = async (fileName: string): Promise<CaseJSON[]> => {
  const filePath = `${process.cwd()}/storage/osceola/searches/${fileName}`
  ...
  if (jsonData) {
    ...
    const mapped = data.map((row: Record<string, string>) => {
      ...
      return extractDataFromJSONRow(rowAsArray)
    })
...
const processAllResults = async () => {
  const files = await fs.readdir(`${process.cwd()}/storage/osceola/searches`)
  ...
  await writeJSONtoFile(`${process.cwd()}/storage/osceola/combined-search-results.json`, {
